---
title: "2026-01-18"
description: "Clerk authentication, waitlist, and localization copy refresh for apps/web"
---
...
| File | Change |
| ---- | ------ |
| `apps/hq/lib/source.ts` | Passed the `meta` collection to `toFumadocsSource` so sidebar folders can read metadata (title/defaultOpen). |

### Technical Decisions

1. **Include Meta Collection**: `toFumadocsSource` must receive both docs and meta; an empty array drops sidebar metadata.
...
## HQ Locale Preservation for External Links
...
| File | Change |
| ---- | ------ |
| `src/components/landing/build-in-public.tsx` | Derived the HQ link from the active locale to preserve language context. |
| `src/components/landing/footer.tsx` | Updated the HQ link to include the locale-aware path (`/en` or `/ko`). |

### Technical Decisions

1. **Locale Fallback**: Mapped non-Korean locales to `/en` since HQ currently supports only `en` and `ko`.
...
| File | Change |
| ---- | ------ |
| `apps/web/messages/*.json` | Refined UI copy across 25 locales, aligned hero CTA text to Read the Manifesto, and removed em dash punctuation. |
| `apps/web/src/components/landing/hero-cta.tsx` | Switched hero CTA key usage from `enterManifesto` to `readManifesto`. |

### Technical Decisions

1. **Local voice per locale**: Adjusted tone and phrasing to read like native product copy while preserving meaning.
2. **Placeholder preservation**: Kept `{count}`, `{position}`, `{title}`, `{index}`, `{year}` intact to avoid runtime issues.
3. **Consistent naming**: Kept brand and proper nouns (Coscientist, GitHub) consistent across locales.
4. **CTA alignment**: Reused the CTA phrasing in the hero to keep a consistent action label.

### Verification

- JSON parse: Pass (`node` parse of all locale message files)
- Build: Not run (copy-only change)

### Next Steps

Optional: have native reviewers spot-check key locales for nuance.
