...
epistêmicos em vez de texto bruto. Tipos comuns de nós incluem:
...
- Definições — significados de termos (acompanhados ao longo do tempo via
  [Deriva de Definição](./definition-drift))
...
confiança (ver [Taxonomia de Status de Alegações](./claim-status-taxonomy)). As
transições são determinadas menos pelo número de refutações e mais pelo tipo de
refutação e se ela tem êxito no nível de escopo, pressuposto, definição ou
método.
