...
這個名稱借用自零信任安全（zero-trust security）：在被證明之前，假設沒有任何東西是安全的。實務上，這表示新的主張會以 `tentative`（暫定）狀態進入，必須能追溯到[來源](./source)與[證據片段](./evidence-span)，並且在被視為既定之前，應先接受[反駁優先搜尋](./rebuttal-first-search)的檢視。
