...
brain）である。自動リンク（バックリンクを含む）と制御された減衰（controlled
decay）を備えた、静的サイト（static-site）の出版システムだ。古いノートは、明示的にエバーグリーン（evergreen）として保持しない限り、徐々に薄れていく。これは「[Superbrains](./superbrain)のための外骨格（Exosuit）」として説明されている。
