...
[은하대백과](./encyclopedia-galactica)에서 영감을 받은 오픈소스 서비스다. 이는
단순히 정보의 [지식 방주](./knowledge-ark) 이상을 지향한다. 인간
[운영자](./operator)와 함께 [사유 노동](./contemplation-labor)을 수행하는
[지적 동반자](./intellectual-companion)다. 포스트-AI 시대에서 그 목표는
[뇌 썩음 방지](./un-brain-rotting)를 지원하는 것, 즉 검증과 행위자성을 회복하는
것이다.

이는 [조성현](./sunghyun-cho)이 만들고 있다. 기원은 간단하다. 보존은 중요하지만,
보존만으로는 이해가 생기지 않는다. Coscientist는 지식을 업데이트 가능하게
...
이 때문에 Coscientist는 Q&A 스타일의 RAG를 넘어선다(참고:
[RAG 한계](./rag-limitations)). Coscientist는 원문 문서 텍스트가 아니라
[논증 행위](./argumentative-act)를 저장하는
[변증법적 그래프](./dialectical-graph)를 유지한다. 그래서 모든 주장은 증거 구간,
출처, 그리고 명시적 관계에 연결될 수 있다(참고:
[변증법적 그래프 노드](./dialectical-graph-nodes) 및
[변증법적 그래프 엣지](./dialectical-graph-edges)).

탐색은 기본적으로 대립적이다. [반례 우선 탐색](./counterexample-first-search),
[최소 컷](./minimum-cut)을 통한 모순 격리, 그리고 명시적
[정의 표류](./definition-drift) 탐지는 초기에 압력을 가해, 지식이 반복에 의해
표류하는 대신 반박을 통해 업데이트되도록 한다(참고:
