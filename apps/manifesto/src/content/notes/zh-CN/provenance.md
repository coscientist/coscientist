...
在[辩证图](./dialectical-graph)中，溯源通过[来源](./source)节点、[证据跨度](./evidence-span)以及 `cites` 边来编码，使归因变得明确，而不是由散文式表述暗示。这能避免在[RAG](./rag-limitations)中常见的摘录截断问题：引用会丢失上下文，而[责任线](./responsibility-line)也随之消失。
