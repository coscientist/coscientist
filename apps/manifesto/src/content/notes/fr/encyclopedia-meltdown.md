...
1. **Génération → citation** : les sorties du modèle sont réingérées dans le corpus puis citées plus tard comme si elles étaient des faits externes.
2. **Liaison → autorité** : les liens internes cessent de fonctionner comme des voies de vérification et deviennent des badges de confiance ; la densité de liens se substitue alors à l’exactitude.
3. **Responsabilité → sans-agent** : lorsqu’il n’est pas clair qui a affirmé quoi et sur quelles bases, la correction devient coûteuse, et le système se stabilise autour d’un faux état stationnaire « maintenu approximativement ».
