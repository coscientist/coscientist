...
description:
  A knowledge graph model that stores claims and relations rather than document
  content
---

A dialectical graph is a knowledge graph that stores argumentative structure
...
propositions; and relations are stored as explicit argumentative bindings. This
is a structural fix for a common [RAG](./rag) failure mode: excerpt truncation.
The evidence span keeps the exact wording and source, the claim records who
asserted what, and a `cites` edge makes attribution explicit instead of implied
by prose.

The graph also separates an inference layer from a narrative layer . A document
is not final truth; it is a snapshot of the graph. Every sentence should be able
to backtrack through claim, scope, assumption, method, data, evidence span, and
