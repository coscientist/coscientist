...
However, RAG has structural limitations for knowledge production. It retrieves text snippets based on similarity, not on argumentative relations. It cannot distinguish support from attack, evidence from opinion, or primary source from restatement. When sources conflict, RAG tends to blend them into smooth summaries rather than surfacing the [contention](./contention). See [RAG Limitations](./rag-limitations) for the full critique.

[Coscientist](./coscientist) goes beyond RAG by maintaining a [Dialectical Graph](./dialectical-graph) that stores [claims](./claim), [evidence spans](./evidence-span), and typed relations. Retrieval becomes "what bears on this claim?" rather than "what is similar to this query?"
