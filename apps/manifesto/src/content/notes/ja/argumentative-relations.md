...
論証関係（argumentative
relations）は、[弁証法グラフ](./dialectical-graph)におけるエッジ種別であり、認識論的オブジェクト（epistemic
objects）が互いにどのように関係するかを符号化します。これは、弁証法グラフをトピック類似性グラフ（topic-similarity
graph）と区別するものです。
...
- `defines`
  — 用語をその定義へ接続する（[定義ドリフト](./definition-drift)を通じて経時的に追跡）
