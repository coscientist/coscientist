...
- [修正 vs ドリフト](./correction-vs-drift) — 誤りを直すコストが、誤りの伝播を放置するコストを上回るとき

## なぜ重要か

システム・ドリフトとは、知識システムが「だいたい維持されている」偽の定常状態のまわりで安定してしまう失敗モードです。修正が高コストなため誤りが残り続け、AI生成コンテンツが学習データを汚染し、システムは徐々にグラウンドトゥルース（ground truth）との接点を失っていきます。
