---
title: 개인 지식의 진화
description: 개인 지식 관리 도구와 아이디어의 역사적 전개
---

개인 지식의 진화는 20세기 중반의 비전에서 현대적 시스템에 이르기까지, 개인
지식을 관리하기 위한 도구와 아이디어가 발전해 온 과정을 추적한다.

## 기초적 비전

- [우리가 생각할 수 있는 대로](./as-we-may-think) —
  [버니바 부시](./vannevar-bush)의 1945년 에세이
- [메멕스](./memex) — 부시가 제안한 개인 지식 기계

## 개념적 프레임워크

- [디지털 브레인](./digital-brain) — 마음을 모방하고 확장하는 시스템
- [디지털 가든](./digital-garden) — 공개적으로 진화하는 상호연결된 노트
- [디지털 정글](./digital-jungle) — 자가 조직화되는 저마찰 지식 공간
- [세컨드 브레인](./second-brain) — 개인 지식 관리 실천법

## 구현

- [프로젝트 알데하이드](./project-aldehyde) — [선현 조](./sunghyun-cho)의 초기
  시도
- [외골격두뇌](./extracranial) — 조의 개인 디지털 브레인 시스템
- [다국어 메멕스](./multilingual-memex) — 자동 번역을 갖춘 언어 인지형 지식
  시스템
- [프로젝트 PIRI](./project-piri) — 취소된 번역 이니셔티브(AI 코딩 에이전트에
  의해 구식화됨)

## 설계 원칙

- [마찰은 적이다](./friction-as-enemy) — 오버헤드를 최소화해 수집을 극대화하기

## Coscientist로의 연결

- [메멕스에서 변증법적 그래프로](./from-memex-to-dialectical-graph)
- [개인 지식에서 제도적 지식으로](./personal-to-institutional-knowledge)
