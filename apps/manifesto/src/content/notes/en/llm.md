...
LLM (Large Language Model) refers to neural network models trained on massive text corpora to predict and generate natural language. Examples include GPT, Claude, Gemini, and Llama. LLMs can perform a wide range of language tasks—summarization, translation, question-answering, code generation—by learning statistical patterns from training data.
...
However, LLMs have fundamental limitations. They optimize for plausible next tokens, not for truth. They can [hallucinate](./hallucination): producing confident, coherent text that is factually wrong. They are susceptible to the [fluency trap](./fluency-trap): smooth prose that masks errors. They share training data, so agreement among models may reflect correlated bias rather than independent [verification](./verification) (see [evidence independence](./evidence-independence)).
