---
title: "2026-01-18"
description: "apps/web Clerk 인증, 대기자 명단, 로컬라이징 카피 개선"
---
...
| 파일 | 변경 사항 |
| ---- | --------- |
| `apps/hq/lib/source.ts` | `toFumadocsSource`에 `meta` 컬렉션을 전달해 사이드바 폴더 메타데이터(title/defaultOpen 등) 반영. |

### 기술적 결정

1. **메타 컬렉션 포함**: `toFumadocsSource`는 docs+meta가 필요하므로 빈 배열을 전달하면 사이드바 메타가 누락됨.
...
| 파일 | 변경 사항 |
| ---- | --------- |
| `src/components/landing/build-in-public.tsx` | 활성 로케일을 사용해 HQ 링크가 언어 컨텍스트를 유지하도록 수정. |
| `src/components/landing/footer.tsx` | HQ 링크에 로케일 경로(`/en` 또는 `/ko`)를 포함하도록 변경. |

### 기술적 결정

1. **로케일 폴백**: HQ가 현재 `en`, `ko`만 지원하므로 기타 로케일은 `/en`으로 매핑.
...
| 파일 | 변경 사항 |
| ---- | --------- |
| `apps/web/messages/*.json` | 25개 로케일 UI 카피를 자연스러운 로컬 톤으로 다듬고 히어로 CTA를 Read the Manifesto로 정렬한 뒤 em dash 표기를 제거. |
| `apps/web/src/components/landing/hero-cta.tsx` | 히어로 CTA 키를 `enterManifesto`에서 `readManifesto`로 변경. |

### 기술적 결정

1. **로컬 톤 우선**: 의미는 유지하면서 실제 제품 카피처럼 읽히도록 문장 구성과 어조를 조정.
2. **플레이스홀더 보존**: `{count}`, `{position}`, `{title}`, `{index}`, `{year}`를 유지해 런타임 오류를 방지.
3. **명칭 일관성**: 브랜드/고유명사(Coscientist, GitHub)를 로케일 간 일관되게 유지.
4. **CTA 정렬**: 히어로와 하단 CTA의 액션 문구를 통일.

### 검증

- JSON 파싱: 통과 (`node`로 전체 로케일 메시지 파싱)
- 빌드: 미실행 (카피 변경)

### 다음 단계

필요 시 주요 로케일에 대해 네이티브 리뷰를 진행.
