...
Les règles de transition de statut définissent ce qui fait passer une [revendication](./claim) d’un état à un autre dans la [taxonomie des statuts de revendication](./claim-status-taxonomy). Les transitions dépendent du type de contestation, pas de leur nombre : un seul [contre-exemple](./counterexample) décisif compte davantage que de nombreuses objections peu convaincantes.
